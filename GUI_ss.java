...
public class GUI_ss extends SudokuSolver {
    ...
    public GUI_ss() {
        frame_box = new JFrame();
        frame_box.setPreferredSize(new Dimension(950, 750));
        ...
        button1.addActionListener(e -> {
            ...
            GUIToSudoku(text_field);
        });

        button2 = new JButton("Clear grid");
        button2.addActionListener(e -> {
            // clearing all JTextFields by setting them ""
            for (int i=0; i<81; i++) {
                text_field[i].setText("");
                text_field[i].setBackground(Color.YELLOW);
                text_field[i].setForeground(Color.BLACK);
            }
        });
        ...
        text_field = getClearedFields();

        panel.setBorder(BorderFactory.createEmptyBorder(30, 30, 10, 30));
        panel.setLayout(new GridLayout(10, 3)); //imported awt module for this
        for (int x=0 ; x<81 ; x++) {
            panel.add(text_field[x]);
        }
        panel.add(button1);
        panel.add(button2);
        panel.add(button3);

        frame_box.add(panel, BorderLayout.CENTER);
        frame_box.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        frame_box.setTitle("Sudoku Solver");
        frame_box.pack();
    ...
    void SudokuToGUI(int[][] solution, int[][] initialSudoku){
        for (int i=0; i<9; i++) {
            for (int j=0; j<9; j++) {
                populateFieldInteractive(text_field[(i*9) + j], solution[i][j], initialSudoku[i][j]);
            }
        }
    }
    ...
    void GUIToSudoku(JTextField[] fields){
        ...
        if (Solver_main.is_Valid_cell(inputPuzzle)){
            int[][] initialSudoku = new int[9][9];
            copySudoku(inputPuzzle.sudoku, initialSudoku);
            int[][] solution = new int[9][9];
            Solver_main.get_firstsolution(inputPuzzle, 0, 0, solution);
